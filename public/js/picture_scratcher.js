const createScratchCard = () => {
    ...
    // Function to get the appropriate bottom image source based on window orientation
    const getBottomImageSrc = () => {
        return window.innerWidth > window.innerHeight ? bottomImages.landscape : bottomImages.portrait;
    };

    // Set bottom image source
    const setBottomImage = () => {
        const bottomImageSrc = getBottomImageSrc();
        bottomImage.src = bottomImageSrc;
        console.log('Bottom image source set to:', bottomImageSrc);
    };
